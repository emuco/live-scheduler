...
- 重置密码后自动使旧登录会话失效
- 管理员重置本人密码后自动刷新当前会话
- Docker Compose 内置可选 Redis 服务和 Redis 持久化 volume
...
```bash
tar -xzf live-scheduler-docker-v1.1.1.tar.gz
cd live-scheduler
```
...
Redis 是可选功能。Docker Compose 已包含 Redis 7 服务，并使用 `live-scheduler-redis-data` volume 持久化。系统未启用 Redis 或连接失败时会自动使用进程内存缓存。
