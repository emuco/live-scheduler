...
- 甘特图增加 15 种邻近色任务配色
- 同类型节目保持相近色系，相邻任务更容易区分
- 任务颜色按节目 ID 稳定分配，刷新后不会变化
- 顶部、登录、注册和安装页面增加 GitHub 项目入口
- 默认继续使用进程内存缓存，不内置 Redis 容器

## v1.1.1 更新内容

- 修复管理员和普通用户密码重置功能
- 密码重置改为按用户 ID 更新，避免邮箱和缓存导致更新失败
- 重置密码后自动使旧登录会话失效
- 管理员重置本人密码后自动刷新当前会话
- Docker 默认使用进程内存缓存，减少部署依赖
...
```bash
tar -xzf live-scheduler-docker-v1.1.2.tar.gz
cd live-scheduler
```
...
系统默认使用进程内存缓存，单实例部署无需安装 Redis。内存缓存重启后会自动重建，不影响 SQLite 中的业务数据。

仅在多实例部署、需要跨实例共享缓存或希望登录限流状态跨重启保留时，才建议连接外部 Redis。
